                ...
                <v-server-table :url="API_URL" :columns="columns" :options="options" ref="table">  
                   <div slot="photo_url" slot-scope="{ row }">
                    ...
                    <div slot="actions" slot-scope="{ row }"> 
                      <router-link :to="'/edit-authors/'+row.id" title="Edit Item" class="btn btn-sm bg-gradient-success">
                          <i class="fa fa-edit" aria-hidden="true"></i>
                      </router-link> 
                      <button v-if="row.status != '2'" @click="deleteAuthors( row.id )" title="Archive Item" class="btn btn-sm bg-gradient-danger">
