        ...
        <div class="row mb-2">
          ...
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
              ...
              <li class="breadcrumb-item active">Recipes List</li>
            </ol>
          </div>
        </div>
      </div><!-- /.container-fluid -->
          ...
          <div class="col-12"> 
            <div class="card">
              <div class="card-header">
                <h3 class="card-title float-left">Manage All Recipes</h3>
                <h3 class="card-title float-right">
                   <router-link to="/add-recipe" class="btn btn-sm btn-primary">
                 ...
                 <v-server-table :url="API_URL" :columns="columns" :options="options" ref="table">  
                   <div slot="photo_url" slot-scope="{ row }">
                    ...
                    <div slot="actions" slot-scope="{ row }"> 
                      <router-link :to="'/edit-recipe/'+row.id" title="Edit Item" class="btn btn-sm bg-gradient-success">
                          <i class="fa fa-edit" aria-hidden="true"></i>
                      </router-link> 
                      <button v-if="row.status != '2'" @click="deleteRecipes( row.id )" title="Archive Item" class="btn btn-sm bg-gradient-danger">
...
export default {
  ...
  data() {
    return {
        ...
        API_URL : config.API_URL+'admin/getRecipes',
        tableData: [],
        options: { 
          headings: {
            photo_url	: 'Image',
    ...
    deleteRecipes(id){
      ...
      _this.$dialog.confirm('Are you sure want to archieved?')
        .then(function(dialog) {
          loader: true;  
            //Delete Code start
            _this.$store.dispatch('deleteRecipes',{ id : id })
